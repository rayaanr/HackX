...
contract SimpleHackathonFactory is Ownable, ReentrancyGuard {
     ...
     */
    function createHackathon(string memory ipfsHash) 
        external 
        onlyOwner 
        nonReentrant 
        returns (uint256 hackathonId, address hackathonAddress) 
    {
        ...
        emit HackathonCreated(hackathonAddress, hackathonId, msg.sender, ipfsHash);
    }
     ...
     */
    function getOrganizerHackathons(address organizer) external view returns (uint256[] memory) {
