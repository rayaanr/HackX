...
import {
  getUserHackathonById,
  updateHackathon,
  deleteHackathon,
} from "@/lib/server/database/hackathons";
    ...
    const processedBody = {
      ...
      registrationPeriod: body.registrationPeriod
        ? {
            ...
            registrationStartDate: body.registrationPeriod.registrationStartDate
              ? new Date(body.registrationPeriod.registrationStartDate)
              : undefined,
            registrationEndDate: body.registrationPeriod.registrationEndDate
              ? new Date(body.registrationPeriod.registrationEndDate)
              : undefined,
          }
        : undefined,
      hackathonPeriod: body.hackathonPeriod
        ? {
            ...
            hackathonStartDate: body.hackathonPeriod.hackathonStartDate
              ? new Date(body.hackathonPeriod.hackathonStartDate)
              : undefined,
            hackathonEndDate: body.hackathonPeriod.hackathonEndDate
              ? new Date(body.hackathonPeriod.hackathonEndDate)
              : undefined,
          }
        : undefined,
      votingPeriod: body.votingPeriod
        ? {
            ...body.votingPeriod,
            votingStartDate: body.votingPeriod.votingStartDate
              ? new Date(body.votingPeriod.votingStartDate)
              : undefined,
            votingEndDate: body.votingPeriod.votingEndDate
              ? new Date(body.votingPeriod.votingEndDate)
              : undefined,
          }
        : undefined,
      schedule: body.schedule
        ? body.schedule.map((slot: Record<string, unknown>) => ({
            ...slot,
            startDateTime: slot.startDateTime
              ? new Date(slot.startDateTime as string)
              : undefined,
            endDateTime: slot.endDateTime
              ? new Date(slot.endDateTime as string)
              : undefined,
          }))
        : undefined,
    };
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (!result.success) {
      if (
        result.error === "Hackathon not found or access denied" ||
        result.error === "Hackathon not found"
      ) {
        return NextResponse.json(
          { error: "Hackathon not found or access denied" },
          { status: 404 },
